	...
	updatetext = function () { 
		if (jQuery('#add_text').val()) {
			if (canvas.getActiveObject().type == 'curvedText' || canvas.getActiveObject().type == 'text') {
				...
				//Kelvin customization
// 				let ctx = canvas.getContext('2d');
// 				ctx.strokeStyle = ctx.fillStyle == '#000000' ? 'white' : 'black';
// 				ctx.lineWidth = 10;
// 				ctx.lineJoin="round";
// 				ctx.fill();
//         ctx.stroke();
				let teste = canvas.getActiveObject().__fill,
				colorTeste = (teste == '#000' || teste == '#000000' || teste == 'rgb(0,0,0)' || teste == 'rgba(0,0,0,1)' || teste == 'black' ? 'white' : 'black');
				canvas.getActiveObject().set({
					stroke: colorTeste,
					strokeLineJoin: 'round',
				});
				canvas.getActiveObject().setShadow('0px 0px 0px rgba(255,17,255,1)');
